...
function generate_pgbadger_box(env_name) {

    return_text = "## Generated by PGConfig " + VERSION + "\n";
    return_text += "# Using '" + env_name + "' log format\n\n";
    ...
    return_text += "log_disconnections = on\n";
    return_text += "log_lock_waits = on\n";
    return_text += "log_temp_files = 0\n";
    return_text += "\nlc_messages = 'C'\n";
...
function generate_pgbadger_stuff () {
    ...
    log_format = ['stderr', 'syslog'];

    log_format.forEach(function(entry) {

        new_id="badger_" + entry;
    ...
    log_format.forEach(function(entry) {
        ...
        new_div=
                $('<div>', { role: "tabpanel", class: "tab-pane fade", id: "badger_" + entry })
                    .append(
                            $('<pre>').append(generate_pgbadger_box(entry))
                        );
            ...
            $('<div>', { class : "col-md-12"})
                .append(
                    $('<div>', { class : "page-header" })
                        ...
                        .append(
                                $('<pre>').append("include = 'pgbadger.conf'")
                            )
                        .append(
                                $('<p>').append('Select your log manager and create the file <code>pgbadger.conf</code> inside your <code>PGDATA</code> directory and add this content:')
                            )
                        .append(usage_container)
                        .append(
                                $('<p>')
                                .append('For more details, see the ')
                                .append(
                                    $('<a>', { href: "http://dalibo.github.io/pgbadger/" }).append('oficial PGBadger Documentation')
                                    )
                                .append('.')
                            )
        )
